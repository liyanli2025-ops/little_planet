...
脚本生成独立的 .env，包含随机注册口令。文件权限为 600，不会被 Git 或 Docker 镜像打包。已有 .env 时会停止，不覆盖已有口令。
...
```bash
sed -n 's/^REGISTRATION_CODE=//p' .env
```

不要把输出或 .env 发进公开聊天、GitHub，也不需要把密码告诉协助者。

两个人用此口令创建不同账号，各选一个角色。然后一人点“账号与配对 → 生成邀请码”，另一人在自己的账号输入邀请码。邀请码 24 小时有效、只能使用一次，生成新码会让旧码失效。
