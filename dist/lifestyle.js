import {createTurningPage} from './page-turn.js?v=4';
...
function book(parent){let g=new T.Group();parent.add(g);g.position.set(0,.4,.27);g.rotation.x=-.28;for(let s of [-1,1]){let cover=box(g,0x547b65,s*.073,0,0,.145,.018,.18);cover.rotation.z=-s*.12;let pages=box(g,0xf5ebcf,s*.071,.012,0,.138,.023,.17);pages.rotation.z=-s*.12;for(let j=0;j<4;j++)box(g,0xd6c9a8,s*.07,.027,-.057+j*.031,.082,.002,.003)}const page=createTurningPage(g);g.visible=false;return {g,page}}
const mainBook=book(body),otherBook=book(compBody);
let active='',together=false,built=false,reading=false,turn=1,tentScale=.03;
...
return {get active(){return active},get paired(){return together},get reading(){return reading},campBuilt(v){built=v;roll.visible=!v;tent.visible=v;blanket.visible=v;lamp.visible=v;glow.visible=v;if(!v&&active==='camp')this.stand()},sit(pair=false,read=true){active='bench';together=pair;reading=read;pose(avatar,body,legs,arms,pair?-.32:0,sitFrame,read);companion.visible=pair;if(pair)pose(companion,compBody,compLegs,compArms,.32,sitFrame,read);mainBook.g.visible=read;otherBook.g.visible=pair&&read;},rest(pair=false){if(!built)return;active='camp';together=pair;reading=false;pose(avatar,body,legs,arms,pair?-.32:0,camp,false);companion.visible=pair;if(pair)pose(companion,compBody,compLegs,compArms,.32,camp,false);mainBook.g.visible=false;otherBook.g.visible=false;},stand(){turn=1;mainBook.page.sheet.visible=false;otherBook.page.sheet.visible=false;if(!active)return;outdoor.add(avatar);avatar.position.set(0,0,0);avatar.quaternion.identity();legs.forEach(g=>g.rotation.x=0);arms.forEach(g=>{g.rotation.x=0;g.rotation.z=0});body.position.y=0;companion.visible=false;mainBook.g.visible=false;otherBook.g.visible=false;active='';together=false;reading=false;},turnPage(){if(!reading||turn<1)return false;turn=0;mainBook.page.sheet.visible=true;otherBook.page.sheet.visible=true;mainBook.page.pose(0);otherBook.page.pose(0);return true},tick(dt,t){const goal=built?1:.03;tentScale+=(goal-tentScale)*(1-Math.exp(-dt*7));tent.scale.setScalar(tentScale);if(active){body.position.y=Math.sin(t*1.3)*.005;compBody.position.y=Math.sin(t*1.3+.5)*.005;if(reading){if(turn<1){turn=Math.min(1,turn+dt/.95);mainBook.page.pose(turn);otherBook.page.pose(turn);if(turn===1){mainBook.page.sheet.visible=false;otherBook.page.sheet.visible=false}}}}glow.intensity=1+Math.sin(t*3)*.08},focus(){if(!active)return null;let center=avatar.getWorldPosition(new T.Vector3());if(together)center.add(companion.getWorldPosition(new T.Vector3())).multiplyScalar(.5);return center.add(new T.Vector3(0,.38,0))},state(){return {activity:active,paired:together,reading,tent:built,pageTurn:turn}}}
}
