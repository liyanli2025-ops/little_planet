...
export function createRainGear({body,head,arms}){
 ...
 let coat=mesh(new T.CylinderGeometry(.16,.245,.38,40),yellow,gear,0,.375,0);coat.scale.z=.82;
 mesh(new T.BoxGeometry(.009,.34,.01),seam,gear,0,.37,.191);
 for(let y of [.27,.37,.47])mesh(new T.SphereGeometry(.013,10,8),seam,gear,.024,y,.20);
 for(let x of [-.12,.12])mesh(new T.BoxGeometry(.071,.045,.018),yellow,gear,x,.31,.19);
 // The hood is open toward the face (+Z); the rim follows the opening.
 let shell=mesh(new T.SphereGeometry(.272,40,24,0,Math.PI*2,1.0,Math.PI-1.0),yellow,hood,0,.015,-.012);shell.rotation.x=Math.PI/2;shell.scale.y=1.02;
 let rim=mesh(new T.TorusGeometry(.235,.018,10,48),yellow,hood,0,.015,.161);rim.scale.y=1.05;
 ...
 let enabled=false;
 return {set(v){enabled=v;gear.visible=hood.visible=v;if(!v)umbrella.visible=false},tick(t,hold=true,shared=false){umbrella.visible=enabled&&hold;if(!enabled||!hold)return;arms[1].rotation.set(-1.13,0,-.11);arms[1].updateMatrix();umbrella.position.copy(new T.Vector3(0,-.21,.02).applyMatrix4(arms[1].matrix));umbrella.rotation.z=Math.sin(t*1.2)*.018;canopy.scale.set(shared?1.42:1,1,shared?1.42:1)},state(){return {raincoat:enabled,umbrella:umbrella.visible}}};
}
